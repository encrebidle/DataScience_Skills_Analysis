...
data_url = 'data_source/fetch_clean_data.csv'
...
x = pd.DataFrame(list(jobs_all.description_tokens.sum()))
skill_count = x.value_counts().rename_axis('keywords').reset_index(name='counts')
...
try: 
    ...
    salary_chart = alt.Chart(salary_df).mark_bar(
        ...
        x=alt.X(salary_column, title="Salary", axis=alt.Axis(format='R,f', labelFontSize=20, titleFontSize=17), bin = alt.BinParams(maxbins = 20)), # bins = len(salary_df[salary_column])/4
